...
import * as feelingPicker from './feelingPicker.ts'
...
export type EmotionPickerState = {
  /**
   ...
   */
  walk: feelingPicker.FeelingPickerState | null
}

export type EmotionPickerAction =
  ...
  | { type: 'answer'; answer: feelingPicker.FeelingPickerAction }
...
export function reduce(
  ...
  switch (action.type) {
    ...
    case 'open': {
      ...
      // walk screen to show.
      return feelingPicker.isDone(walk)
        ? close(opened, walk)
        : { ...opened, walk }
    }
    ...
    case 'answer': {
      if (!state.walk) return state
      const walk = feelingPicker.reduce(state.walk, action.answer)
      ...
      return feelingPicker.isDone(walk) ? close(state, walk) : { ...state, walk }
    }
 ...
 */
function shownAsCard(state: EmotionPickerState): Set<string> {
  const shown = new Set(
...
export function cards(state: EmotionPickerState): Visited[] {
 ...
 */
export function pills(state: EmotionPickerState): Choice[] {
  const shown = shownAsCard(state)
...
export function counts(state: EmotionPickerState): {
  met: number
  unmet: number
} {
 ...
 */
export function chosen(state: EmotionPickerState): Visited[] {
