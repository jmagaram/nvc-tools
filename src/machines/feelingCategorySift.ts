 ...
 */
/**
 * A few words of someone's own about one feeling. Never blank — an emptied box
...
function notesInSourceOrder(
  ...
  return words
    .filter((feeling) => written.has(feeling.word))
    .map((feeling) => ({
      word: feeling.word,
...
export function init(
  ...
  return {
    ...
    anchor: null,
    preview: null,
    notes: notesInSourceOrder(category.feelings, alreadyNoted),
    noting: null,
  }
}
...
/**
 * The feeling the gloss strip is showing: what the pointer is dwelling on if
...
export function gloss(state: FeelingCategorySiftState): Feeling | null {
  ...
  return state.words.find((feeling) => feeling.word === word) ?? null
}

/**
 * The feeling everything on this screen applies to — the note button, the `n`
 ...
 */
export function anchored(state: FeelingCategorySiftState): Feeling | null {
  return state.words.find((feeling) => feeling.word === state.anchor) ?? null
}
 ...
 */
export function tabbable(state: FeelingCategorySiftState): string | null {
