...
import { gloss, isMarked, noteFor } from '../machines/needCategorySift.ts'
import type {
...
export default function NeedCategorySift({ state, onAction }: Props) {
  ...
  // sweep across a long category offering nothing.
  ...
  /* `n` writes about whatever the strip is showing, which is whatever the
     ...
     belongs to whoever is hosting this. The drawer is a sibling of this screen
     rather than a child, so typing an `n` into the box never arrives here. */
  ...
  return (
    ...
    >
      {/* 'data-sift' is how a host finds this screen to focus it — see
          ...
          aria attribute, for the reason `NeedPrompt` gives. The only key it
          takes is `n`: the arrows answer a prompt, and here they would fight
          the browser's own way through a run of buttons. */}
      <div
