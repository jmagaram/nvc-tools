    ...
    >
      ...
      >
        ...
        {/* On a touch screen there is no hover and the tap does both showing and
            ...
            A listbox rather than a run of toggle buttons: a list of words of
        ...
        >
          {state.words.map((need) => (
            <NeedPill
              ...
              noted={visibleNote(state, need.word) !== null}
              tabbable={need.word === tabbable(state)}
              anchored={need.word === anchor}
              describedBy={glossId}
              onClick={() => onAction({ type: 'toggle', word: need.word })}
              onNote={() => onAction({ type: 'note', word: need.word })}
              onAnchor={() => onAction({ type: 'anchor', word: need.word })}
              onPreview={() => {
                if (!noting) hover.enter(need.word)
              }}
            />
          ))}
        </div>
